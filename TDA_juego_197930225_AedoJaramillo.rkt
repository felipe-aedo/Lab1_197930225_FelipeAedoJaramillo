...
(provide juego juego? juego-agregar-jugador juego-lanzar-dados juego-actualizar-jugador juego-actualizar-jugadores juego-obtener-jugador-i
         ...
         jugador-agregar-carta banco-paga evento-cumpleaños evento-impuesto)
...
;Dom :game(juego) X dinero (int)
;rec :juego
(define (juego-actualizar-dineroBanco game dinero)
  (list-set game 2 dinero)
  )
...
(define (juego-actualizar-jugadores game players)
  (if (= 1 (length players))
      (juego-actualizar-jugador game (car players))
...
(define (juego-actualizar game player prop mode)
  (cond
    ...
    ((and (= 1 mode) (jugador-puede-comprar? player prop))
     ;modo 1: si es posible comprar la propiedad, actualiza player y propiedad efectuando compra de la propiedad
     (begin (display "Has comprado una propiedad.\n")
      (juego-actualizar-dineroBanco
      (juego-actualizar-propiedad
       (juego-actualizar-jugador game
                                  (jugador-comprar-propiedad player prop))
       (propiedad-set-dueño prop (jugador-get-id player)))
      (+ (juego-get-dineroBanco game)
         (propiedad-get-precio prop))))
      )
    ((and (= 2 mode)(>= (jugador-get-dinero player) (propiedad-get-precio prop)))
     ;modo 2: si es posible, construye una casa y actualiza player y propiedad     
     (begin (display "Has comprado una casa para tu propiedad.\n")
      (juego-actualizar-dineroBanco
      (juego-actualizar-propiedad
       (juego-actualizar-jugador game
                                  (jugador-comprar-casa player prop))
       (propiedad-construir-casa prop game))
      (+ (juego-get-dineroBanco game)
         (propiedad-get-precio prop))))
      )
    ((and (propiedad? prop) (= 3 mode))
     ;modo 3: si es posible, construye hotel y actualiza player y propiedad.
     (juego-actualizar-propiedad ;actualizar propiedad
      game
       (propiedad-construir-hotel prop game) ; aca se verifica si se puede construir el hotel
       )
     )
    (else
      (juego-actualizar-jugador game player)) ;si no es posible comprar (posiblemente por el precio) se retorna el juego con el player movido
    )
  )
...
;Cobra impuesto a un jugador
;Dom: player(jugador) X Game (juego)
;Rec: juego
(define (jugador-cobrar-impuesto player game)
  (juego-actualizar-dineroBanco
   (juego-actualizar-jugador
    game
    (jugador-set-dinero
     player
     (- (jugador-get-dinero player) (juego-get-tasaImpuesto game))))
   (+ (juego-get-dineroBanco game) (juego-get-tasaImpuesto game)))
  )
...
(define (jugador-mover player game dados)
  (if (jugador-completo-vuelta? player game dados) ;verificar si completa una vuelta
...
;obtiene la posicion de la carcel
;dom: game(juego)
;rec juego
(define (juego-get-carcel game)
  (cdr (car(filter (lambda(par) (eq? 'carcel (car par))) (tablero-get-casillasEspeciales (juego-get-tablero game)))))
  )


;/////////////////////////////////FUNCIONES CARTAS/////////////////////////////////////////////////
;encarcela (mueve hasta la carcel y cambia el valor booleano correspondiente)
; dom: game(juego) X player(jugador)
; rec: juego
(define (jugador-encarcelar game player)
  (begin (display "Jugador encarcelado.\n")
  (juego-actualizar-jugador game (jugador-set-posicion (jugador-switch-carcel player) (juego-get-carcel game))))
  )


; Descripción: lleva al player a la salida
; Dom: player (jugador)
; Rec: juego
(define (jugador-ir-salida game player)
  (begin (display "Jugador es llevado a la salida.\n")
  (juego-actualizar-jugador game (jugador-set-posicion player 0))
  )
  )

;paga al player y actualiza juego
; dom : game (juego) X player(jugador)
; Rec : juego
(define (banco-paga game player)
  (begin (display "Es tu dia de suerte, banco paga $50.\n")
  (juego-actualizar-dineroBanco (juego-actualizar-jugador game (jugador-set-dinero player (+(jugador-get-dinero player) 50))) (-(juego-get-dineroBanco game) 50)))
  )

; Descripción: agrega una carta para salir de la carcel 
; dom: player(jugador)
; rec: juego
(define (jugador-agregar-carta game player)
  (if (jugador? player)
      (begin (display "Has ganado una carta para salir de la carcel.\n")
      (juego-actualizar-jugador game (jugador (jugador-get-id player) (jugador-get-nombre player) (jugador-get-dinero player) (jugador-get-propiedades player)
...
;Evento cumpleaños, cobra 10 a cada player para darselo al cumpleañero
;dom: game (juego) X player(jugador)
;rec: juego
(define (evento-cumpleaños game player)
  (begin (display "Feliz cumpleaños, recibes $10 de cada jugador.\n")
  (juego-actualizar-jugadores
   (juego-actualizar-jugador
    game
    (jugador-set-dinero
     player
     (+ (jugador-get-dinero player)
        (* (- (length (juego-get-jugadores game)) 1) 10))))
   (map
    (lambda (p)
      (jugador-set-dinero p (- (jugador-get-dinero p) 10)))
    (juego-restoJugadores game (jugador-get-id player))))
  )
  )


;Evento impuesto (carta impuesto)
;dom: game(juego( X player(jugador)
;rec: juego
(define (evento-impuesto game player)
  (begin (display "Toca pagar impuestos, pierdes $100.\n")
  (juego-actualizar-dineroBanco
   (juego-actualizar-jugador
    game
    (jugador-set-dinero player (- (jugador-get-dinero player) 100)))
   (+ (juego-get-dineroBanco game) 100))
  )
  )
;/////////////////////////////////////////////////////////////////////////////////////
...
; Comprueba si un jugador completo una vuelta, para cobrarle los impuestos.
; Dom: player(jugador) X game (juego) X Dados (lista de integer)
; Rec: Bool
(define (jugador-completo-vuelta? player game Dados)
  (<(juego-get-ultimaPosicion game) (+ (jugador-get-posicion player) (apply + Dados)))
  )
...
; Funcion principal para ejecutar un turno. Su logica se ramifica inicialmente, considerando las posibles casillas en las que se cae. a partir de ahí se evalúan las flags.
; Dom : game (juego) X lista dados (lista de integer) X compCons? (bool) X consHotel? (bool) X pagarMc (bool) X usarTsc (bool)
; Rec : juego
(define (juego-jugar-turno game listaDados compCons? consHotel? pagarMc usarTsc)
  (if (not (jugador-esta-en-bancarrota (juego-obtener-jugador-actual game)))
      (if (not(jugador-estaencarcel (juego-obtener-jugador-actual game))) ;si no esta en la carcel el jugador de turno, se mueve libremente.
          (cond
            ...
            ((propiedad? (tablero-get-posicionX (juego-get-tablero game) (jugador-get-posicion (jugador-mover (juego-obtener-jugador-actual game) game listaDados))))
             (begin (display "Has caido en una propiedad.\n")
                                      ...
                                      (juego-avanzar-turno
                                       (juego-actualizar game
                                                         ...
                                                         (juego-obtener-propiedad game (jugador-get-posicion (jugador-mover
                                                                                                              (juego-obtener-jugador-actual game) game listaDados))) 2) ;modo consCasa
                                       )
            ...
            ((eq? (tablero-get-posicionX (juego-get-tablero game) (jugador-get-posicion (jugador-mover (juego-obtener-jugador-actual game) game listaDados))) 'carcel)
             (begin (display "Has sido encarcelado.\n")
            ...
            (else ;////RAMA 4, cae en una carta (o todas las flags eran false)
             (if (symbol? (tablero-get-posicionX (juego-get-tablero game) (jugador-get-posicion (jugador-mover (juego-obtener-jugador-actual game) game listaDados))))
                 (begin(display "Has caído en una carta.\n")
                       (if (eq? 'suerte
                                (tablero-get-posicionX (juego-get-tablero game) (jugador-get-posicion (jugador-mover (juego-obtener-jugador-actual game) game listaDados))))
                           ; sacar carta suerte y aplicarla
                           (juego-avanzar-turno ((carta-get-accion(juego-extraer-carta game "suerte")) (juego-actualizar-jugador game (jugador-mover (juego-obtener-jugador-actual game) game listaDados))
                                                                                                       (jugador-mover (juego-obtener-jugador-actual game) game listaDados))
                                                )
                           (juego-avanzar-turno ((carta-get-accion(juego-extraer-carta game "comunidad")) (juego-actualizar-jugador game (jugador-mover (juego-obtener-jugador-actual game) game listaDados))
                                                                                                          (jugador-mover (juego-obtener-jugador-actual game) game listaDados))
                                                )
                           ))
                 (if (null? (propiedad-get-dueño (juego-obtener-propiedad game (jugador-get-posicion (jugador-mover (juego-obtener-jugador-actual game) game listaDados))))) 
          ...
          (cond ;
            ;/// PAGAR MULTA
            (pagarMc (juego-actualizar-dineroBanco (juego-avanzar-turno (juego-actualizar-jugador game (jugador-pagar-multa-carcel (juego-obtener-jugador-actual game)))) (+ (juego-get-dineroBanco game) 500)))
        
            ;/// USAR CARTA SALIR CARCEL
            ((and usarTsc (> (jugador-get-cartas (juego-obtener-jugador-actual game)) 0)) ;intentar usar tarjeta
             ((juego-avanzar-turno (juego-actualizar-jugador game (jugador-switch-carcel (jugador-gastar-carta (juego-obtener-jugador-actual game))))) ;desencarcelar y usar tarjeta
              (if (jugada-perfecta? listaDados) ;si no tenia cartas, verificar si sale por jugada perfecta
                  (juego-avanzar-turno (juego-actualizar-jugador game (jugador-switch-carcel (juego-obtener-jugador-actual game))))
                  (juego-avanzar-turno game)
                  ))
             )
            (else ;revisar si saca dobles, triples, cuadruples ,etc (sale de la carcel si saca la jugada perfecta, sigue encarcelado en caso contrario)
             (if (jugada-perfecta? listaDados)
                 (juego-avanzar-turno (juego-actualizar-jugador game (jugador-switch-carcel (juego-obtener-jugador-actual game))))
                 (juego-avanzar-turno game)
                 )
             )
      ...
      (begin (display "Jugador en bancarrota, siguiente turno.\n") (juego-avanzar-turno game))
      )
  )
